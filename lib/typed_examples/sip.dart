...
import 'package:janus_test/conf.dart';
...
class _SipExampleState extends State<TypedSipExample> {
  ...
  Future<void> localMediaSetup() async {
    MediaStream? temp =
        await sip?.initializeMediaDevices(useDisplayMediaDevices: false);
    localStream = temp;
  }
    ...
    callDialog = await showDialog(
        ...
        builder: (context) {
          return StatefulBuilder(builder: (context, _state) {
            ...
            return AlertDialog(
              title: const Text(
                  "Call Registered User or wait for user to call you"),
              content: Column(
                mainAxisSize: MainAxisSize.min,
                children: [
                  TextFormField(
                    decoration:
                        const InputDecoration(labelText: "sip URI to call"),
